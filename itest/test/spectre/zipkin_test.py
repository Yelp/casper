...
def _call_with_zipkin(
    ...
    if no_cache:
        zipkin_headers['Pragma'] = 'spectre-no-cache'
...
def _assert_no_span_in_logs():
    assert load_zipkin_spans(SYSLOG_FILE) == []


def _check_span_logs(trace_id, span_id, parent_span_id, num_lines=1, cache_status=None):
...
class TestZipkinLogging(object):
    ...
    def test_logs_zipkin_info_to_error_log(self, clean_log_files):
        trace_id, span_id, parent_span_id = _get_random_zipkin_ids()
        _call_with_zipkin(trace_id, span_id, parent_span_id, sampled='1')
        _check_span_logs(trace_id, span_id, parent_span_id)

    def test_still_log_even_if_not_sampled(self, clean_log_files):
        trace_id, span_id, parent_span_id = _get_random_zipkin_ids()
        _call_with_zipkin(trace_id, span_id, parent_span_id, sampled='0')
        _check_span_logs(trace_id, span_id, parent_span_id)
    ...
    def test_propagates_zipkin_headers(self, clean_log_files):
        ...
        """
        trace_id, span_id, parent_span_id = _get_random_zipkin_ids()
        response = _call_with_zipkin(trace_id, span_id, parent_span_id, sampled='1')
        backend_headers = response.json()['received_headers']
        _check_backend_headers(trace_id, span_id, parent_span_id, backend_headers)
    ...
    def test_propagates_zipkin_headers_if_not_sampled(self, clean_log_files):
        ...
        """
        trace_id, span_id, parent_span_id = _get_random_zipkin_ids()
        response = _call_with_zipkin(trace_id, span_id, parent_span_id, sampled='0')
        backend_headers = response.json()['received_headers']
        _check_backend_headers(trace_id, span_id, parent_span_id, backend_headers)
    ...
    def test_logs_zipkin_span_if_cached(self, clean_log_files):
        ...
        trace_id, span_id, parent_span_id = _get_random_zipkin_ids()
        uncached_response = _call_with_zipkin(
            trace_id,
            span_id,
            parent_span_id,
            url='/long_ttl/zipkin',
        ...
        trace_id, span_id, parent_span_id = _get_random_zipkin_ids()
        cached_response = _call_with_zipkin(
            trace_id,
            span_id,
            parent_span_id,
            url='/long_ttl/zipkin',
        ...
        _check_span_logs(trace_id, span_id, parent_span_id, num_lines=2)
    ...
    def test_dont_emit_span_if_no_headers(self, clean_log_files):
        ...
        _assert_no_span_in_logs()


class TestZipkinCacheStatusTag(object):

    def test_cached(self, clean_log_files):
        """Ensure cache_status is properly recorded in case of miss or hit."""
        # Hit an uncached endpoint
        # I've added `test=test_cached` otherwise other tests might pollute the cache
        trace_id, span_id, parent_span_id = _get_random_zipkin_ids()
        uncached_response = _call_with_zipkin(
            trace_id,
            span_id,
            parent_span_id,
            url='/long_ttl/zipkin?test=test_cached',
        )
        assert uncached_response.headers['Spectre-Cache-Status'] == 'miss'
        _check_span_logs(trace_id, span_id, parent_span_id, cache_status='miss')
        ...
        assert cached_response.headers['Spectre-Cache-Status'] == 'hit'
        _check_span_logs(trace_id, span_id, parent_span_id, num_lines=2, cache_status='hit')

    def test_non_cacheable(self, clean_log_files):
        """Ensure cache_status is properly recorded if the endpoint is not cacheable."""
        # Hit an uncached endpoint
        trace_id, span_id, parent_span_id = _get_random_zipkin_ids()
        uncached_response = _call_with_zipkin(
            trace_id,
        ...
        assert uncached_response.headers['Spectre-Cache-Status'] == 'non-cacheable-uri (backend.main)'
        _check_span_logs(trace_id, span_id, parent_span_id, cache_status='non-cacheable-uri (backend.main)')

    def test_no_cache_header(self, clean_log_files):
        """Ensure cache_status is properly recorded if a no-cache header is passed."""
        # Hit an uncached endpoint
        trace_id, span_id, parent_span_id = _get_random_zipkin_ids()
        uncached_response = _call_with_zipkin(
            trace_id,
        ...
        assert uncached_response.headers['Spectre-Cache-Status'] == 'no-cache-header'
        _check_span_logs(trace_id, span_id, parent_span_id, cache_status='no-cache-header')
